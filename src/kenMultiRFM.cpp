...
// pair-sampling version (CRAN-compliant & Thread-safe)
// [[Rcpp::export]]
arma::mat SK_pair_MC_cpp_parallel(const arma::mat& X,
  ...
  // 【CRAN 核心修改】：在并行环境外，安全使用受 R 语言 seed 控制的随机数
  arma::uvec i_idx = arma::randi<arma::uvec>(m, arma::distr_param(0, n - 1));
  arma::uvec j_idx = arma::randi<arma::uvec>(m, arma::distr_param(0, n - 1));

  for(int k = 0; k < m; k++) {
    while(j_idx(k) == i_idx(k)) {
      j_idx(k) = arma::randi<arma::uvec>(1, arma::distr_param(0, n - 1))(0);
    }
  }
