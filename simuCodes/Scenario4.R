#### In this simulatio scenario, we consider the influence of  data source number on the estimation


gendata_simu3_robust <-function (seed = 1, nvec = c(100,300), p = 50, q = 3,
                                 qs= rep(2, length(nvec)),
                                 fac.type = c("gaussian", 'mvt'), err.type=c("gaussian", 'mvt'),
                                 rho = c(1,1), sigma2_eps=0.1, nu=3){
  ...
  rho <-0.1  # Change rho <- 0.1,0.2 or 0.3
  
  for(s in 1:S){
    n <- nvec[s]
    
    if(err.type == 'gaussian'){
      innov <- MASS::mvrnorm(n+100, mu=rep(0,p), Sigma=diag(p)*sigma2_eps)
      eps_ar <- apply(innov, 2, function(x) filter(x, filter = rho, method = "recursive"))
      epsi <- tail(eps_ar, n)
      
    } else if(err.type == 'mvt'){
      innov <- mvtnorm::rmvt(n+100, sigma=sigma2_eps*diag(p), df=nu)
      eps_ar <- apply(innov, 2, function(x) filter(x, filter = rho, method = "recursive"))
      epsi <- tail(eps_ar, n)
    }
    
    ##epsi <- mvtnorm::rmvt(n, sigma =sigma2_eps* diag(p), df=nu)这是关于simu4的：是否修改？怎么生成自回归的，系数调整
...
setwd(dir.name)
source("helpfunc.R")
number.source.list <- list(c(100, 200), c(100, 200, 300), c(100, 200, 300, 500))
# i <- commandArgs(TRUE) %>% as.integer()
j <- 3 
nvec <- number.source.list[[j]];
nu <- 2 
p <- 400
q <- 3; qs <- rep(2, length(nvec)); S <- length(nvec)
sigma2_eps <- 1
N <- 100 
method_run <- 'All'
...
for(i in 1:N){
  ...
  print("MultiRFM")
  message("i = ", i)
  datList <- gendata_simu3_robust(seed=i, nvec=nvec, p=p, q=q, qs=qs,
