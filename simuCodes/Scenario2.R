#### In this simulatio scenario, we consider the outliners influence on the estimation

gendata_simu2_robust <-function (seed = 1, nvec = c(100,300), p = 50, q = 3,
                                 qs= rep(2, length(nvec)), out.number = 100,
                                 fac.type = c("gaussian", 'mvt'), err.type=c("gaussian", 'mvt'),
                                 rho = c(1,1), sigma2_eps=0.1, nu=1){
  
  # seed = 1; nvec = c(100,300); p = 50; q = 3
  # qs= rep(2, length(nvec))
  # rho = 1; sigma2_eps=0.1;out.prop = 0.1
  # fac.type = c("gaussian");  err.type= 'mvt'
  ...
  for(s in 1:S){
    ...
    Xs <- matrix(bmu0[,s], nrow=n, ncol=p, byrow = TRUE) + FH %*% t(AB1) + epsi
    
    ### Add outliners
    idx <- sample(prod(dim(Xs)), out.number)
    message('There are ', out.number, ' outliners in these locations: ', toString(idx))
    Xs[idx] <- Xs[idx]*10 #这里改为乘10
    Xlist[[s]] <- Xs
  }
...
nu <- nu.vec[j]
p <- 400
nvec <- c(250,300);  q <- 3;qs <- c(2,2); S <- length(nvec)
sigma2_eps <- 1
N <- 100 
out_number=50  # Change out_number
method_run <- 'All'
...
for(i in 1:N){
  ...
  print("MultiRFM")
  message("i = ", i)
  datList <- gendata_simu2_robust(seed=i, nvec=nvec, p=p, q=q, qs=qs,
                                  rho=c(4,4), err.type='mvt', out.number = out_number,
  ...
  sapply(metricList, colMeans, na.rm=T)
  sapply(metricList, colSD)
  save(metricList, file=paste0(dir.name,"_","out_number",out_number,"nsum", sum(nvec),"nu",paste0(nu, collapse = "_"),
